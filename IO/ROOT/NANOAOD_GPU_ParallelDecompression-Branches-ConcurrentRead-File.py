...
def get_branch_buffer_metadata(filepath, branch, counts_):
    def get_data(i):
...
def GPU_all_baskets_to_array(filepath, branches, counts_call = False):
    arrays = {}
    branches_metadatas = {}
    output_buffers = []
    ...
    if counts_call == False:
        ...
        counts = GPU_all_baskets_to_array(filepath, count_branches, counts_call = True)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for i, branch in enumerate(branches):
            futures.append(
                executor.submit(
                    get_branch_buffer_metadata, filepath, branch, counts
                )
            )
...
branches = [tree["Muon_pt"], tree["Muon_eta"], tree["Muon_phi"], 
            tree["Jet_pt"], tree["Jet_eta"], tree["Jet_phi"], 
            tree["FatJet_pt"], tree["FatJet_eta"], tree["FatJet_phi"], 
            tree["Electron_pt"], tree["Electron_eta"], tree["Electron_phi"]]

myarrays = GPU_all_baskets_to_array(filename, branches)